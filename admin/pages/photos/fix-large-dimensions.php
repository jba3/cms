<?php
	$pageheader = "Page Photos - Adjusting size on large images";
	...
	dbOpen();
		...
		$qryPhotos = dbSelect("photoID,imageFilename from cms_pagePhotos where validLarge=0 and pageID=" . $_GET["pageID"] . ' LIMIT ' . $maxImages);
		$qryPhotosLeft = dbSelect("photoID from cms_pagePhotos where validLarge=0 and pageID=" . $_GET["pageID"]);
		...
		while ($rowPhotos = mysql_fetch_assoc($qryPhotos)) {
			...
			$image->save($dirLarge . $rowPhotos['imageFilename']);
			dbUpdate("cms_pagePhotos set validLarge=1 where photoID=" . $rowPhotos['photoID']);
	...
	if (mysql_num_rows($qryPhotos) == $maxImages){
		...
		echo "	location.href='/admin/pages/photos/fix-large-dimensions.php?pageID=" . $_GET["pageID"] . "';";
		echo '</script>';
	}
